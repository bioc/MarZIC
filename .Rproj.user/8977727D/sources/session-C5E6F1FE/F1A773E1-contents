mediation_effect_cal <- function(para_vec, x_1, x_2, confound_mat) {
  ...
  delta_1_x2 <- gamma_0 + gamma_1 * x_2 + sum(gamma_conf * conf_mean)
  mu_x2 <- alpha_0_vec + alpha_1_vec * x_2 + sum(alpha_conf * conf_mean)
  # mu_2_x2<-alpha_02+alpha_12*x_2

  delta_1_x1 <- gamma_0 + gamma_1 * x_1 + sum(gamma_conf * conf_mean)
  mu_x1 <- alpha_0_vec + alpha_1_vec * x_1 + sum(alpha_conf * conf_mean)
